package constant_full_and_incre
...
object Const_mcht_full {
  ...
  val cjlog_tmp1 =
    s"""
      ...
      |	(CASE WHEN cjlog_tmp.oldtaxno='null' or cjlog_tmp.oldtaxno='' THEN '' ELSE cjlog_tmp.oldtaxno END) AS oldtaxno
      |from
      |(
      |select
      |	taxno,
      ...
      |)cjlog_tmp where cjlog_tmp.rank=1 and cjlog_tmp.mflag=1
    """.stripMargin
/*
  val cjlog_tmp =
    """
  ...
  //处理两条记录中taxno相同，或者一条记录taxno与另一条记录oldtaxno相同的记录
  val cjlog_tmp =
    """
      |select a.taxno1 as taxno,a.oldtaxno1 as oldtaxno from (
      |select c1.taxno as taxno1,c1.oldtaxno as oldtaxno1,c2.taxno as taxno2,c2.oldtaxno as oldtaxno2 from cjlog_tmp1 c1
