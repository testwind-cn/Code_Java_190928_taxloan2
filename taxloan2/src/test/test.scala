...
import com.plj.scala.tools.{loadString, spark}
import org.apache.spark.{SparkConf, SparkContext}
import org.apache.spark.sql.hive.HiveContext
import taxloan2.Const.{Const_Common, Const_counterparty, Const_counterparty_classify, Const_mcht_full, Const_mcht_incre, Const_statistics_crossmonth, Const_statistics_month, Const_table_export}
...
//    hc.sql(Const_Common.truncate_cjlog_tmp).show(10)
    ...
    if ( isIncre ) {
      ...
      println(Const_mcht_incre.insert_cjlog_tmp) // 增量是自己的
      hc.sql(Const_mcht_incre.insert_cjlog_tmp).show(10)
    ...
    if ( isIncre ) {
      println(Const_mcht_incre.insert_saleinvoice_tmp1) // 增量是自己的
      hc.sql(Const_mcht_incre.insert_saleinvoice_tmp1).show(10)
    } else {
      println(Const_mcht_full.insert_saleinvoice_tmp1) // 增量是自己的
    ...
    if ( isIncre ) {
      println(Const_mcht_incre.insert_saleinvoice_tmp2) // 增量是自己的
      hc.sql(Const_mcht_incre.insert_saleinvoice_tmp2).show(10)
    } else {
    ...
    println(Const_mcht_full.insert_saleinvoice_tmp) // 增量是自己的,但和全量一样
    ...
    println(Const_mcht_full.insert_mcht_tax) // 增量和全量SQL这步相同
    ...
    if ( isIncre ) {
      ...
      println(Const_mcht_incre.insert_cross_month_tmp) // 增量全量使用不同的,使用各自的
      hc.sql(Const_mcht_incre.insert_cross_month_tmp).show(10)
    } else {

      println(Const_mcht_full.insert_cross_month_tmp) // 增量全量使用不同的,使用各自的
